############################################################
# Heterogeneous 3D PDN bottom pass
# Bottom: Nangate45-style
############################################################
...
lassign [derive_mesh_pitch_offset $core_width 0.84 $m4_pitch 0.0] bot_m4_pitch bot_m4_offset
lassign [derive_mesh_pitch_offset $core_height 1.4 $m7_pitch 2.0] bot_m7_pitch bot_m7_offset

or_rebuild_rows_for_site $::env(BOTTOM_SITE) bottom
set bot_rail_pitch [get_row_height_um 1.4]
puts "INFO: Using hetero bottom followpins pitch = $bot_rail_pitch um"
...
add_pdn_stripe -grid {BOT} -layer {M1} -width {0.17} -pitch $bot_rail_pitch -offset {0} -followpins -nets {BOT_VDD BOT_VSS}
add_pdn_stripe -grid {BOT} -layer {M4} -width {0.84} -pitch $bot_m4_pitch -offset $bot_m4_offset -nets {BOT_VDD BOT_VSS}
add_pdn_stripe -grid {BOT} -layer {M7} -width {1.4} -pitch $bot_m7_pitch -offset $bot_m7_offset -nets {BOT_VDD BOT_VSS}
