############################################################
# ASAP7 3D PDN upper pass
############################################################

puts "INFO: Start ASAP7 upper PDN..."
...
puts "INFO: Using ASAP7 upper followpins pitch = $rail_pitch um"
...
add_pdn_stripe -grid {TOP} -layer {M1_m} -width {0.018} -pitch $rail_pitch -offset {0} -followpins -nets {TOP_VDD TOP_VSS}
add_pdn_stripe -grid {TOP} -layer {M2_m} -width {0.018} -pitch $rail_pitch -offset {0} -followpins -nets {TOP_VDD TOP_VSS}
add_pdn_stripe -grid {TOP} -layer {M3_m} -width {0.234} -spacing {0.072} -pitch {5.4} -offset {0.300} -nets {TOP_VDD TOP_VSS}
add_pdn_stripe -grid {TOP} -layer {M6_m} -width {0.288} -spacing {0.096} -pitch {5.4} -offset {0.513} -nets {TOP_VDD TOP_VSS}
...
#   -stripe_offsets {} \
#   -stripe_spacings {} \
#   -connect_layers {{M4_m M6_m}}

# pdngen
puts "INFO: Done ASAP7 upper PDN."
